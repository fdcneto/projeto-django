...
class UsuarioSerializer(serializers.ModelSerializer):
    ...
    senha = serializers.CharField(
        min_length=8, max_length=50, write_only=True  # Não aparece na saida
    )
    ...
    def validate_email(self, value):
        ...
        if Usuario.objects.filter(email=email_criacao).exists():
            raise serializers.ValidationError("Este email já está cadastrado")
        return email_criacao

    def validate_senha(self, value):
        if value.isdigit():
            raise serializers.ValidationError("Senha não pode ser apenas números")
        return value
